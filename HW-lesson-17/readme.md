...
### 2.4. Выполнение анализа файла audit.log с помощью утилиты – sealert. 
...
После установки модуля для изменения политики SE Linux, выполняем удаленное внесение изменений в зону ddns.lab. При внесении изменений формируется ошибка: update failed SERVFAIL

![picture](pic/pic12.png)

Для того чтобы убититься что SE Linux не мешает работе сервису named выполняем перезагрузку сервера ns01, очищаем файл (выполняем truncate) /var/log/audit/audit.log и повторно выполняем  удаленное внесение изменений в зону ddns.lab.

![picture](pic/pic14.png)

После чего на сервере ns01 выполняем команду sealert –a /var/log/audit/audit.log
Результат - ошибок SE Linux нет. (см. ниже)
![picture](pic/pic15.png)

После выполнения стандартных проверок на доступность хостов по сети и нормальной работы сервиса named выполнил команду journalctl -u named и обноружил следующие:

![picture](pic/pic16.png)

Вопрос Google что делать с данной ошибкой? Нашел следующий ответ.

	В редких случаях, можно получить ошибку "journal rollforward failed: journal out of sync with zone" при работе с DNS сервером BIND. Вы исправили что-либо в файле зоны, выполнили rndc reload, а исправление не применились! При просмотрел логов, вы увидели вышеописанную ошибку. Что делать?
	Исправление данной ошибки до смешного простое. Посмотрите внимательно, рядом с файлом вашей зоны, находится файл с таким же именем, но с раширением .jnl. Просто удалите его и снова выполните rndc reload или перезапуск DNS-сервера.

В каталоге /etc/named/dynamic удалил файл named.ddns.lab.view1.jnl.
После чего выполнил рестарт сервиса named.
systemctl restart named
Выполнил обновление зоны с ВМ client:

![picture](pic/pic17.png)

Запрос прошел.
После чего проверил в journalctl выполнение запроса на обновление зоны – и о чудо – dns зона обновилась

![picture](pic/pic18.png)
