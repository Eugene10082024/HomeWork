...
Перед развертыванием elk содержимое каталога provisioning_elk переносим в /etc/ansible управляющей ВМ (192.168.11.2) , предварительно все удалив из него.
...
### 2.4. Выполняем подключение к elk через kibana (http://192.168.11.232:5601) и первоначальную настройку elk для анализа logs nginx.

![picture](pic/pic0.png)

#### 2.5. Проверяем работу elk по сбору logs nginx.

#### 2.5.1 Проверяем доступность nginx (http://192.168.11.231)

![picture](pic/pic1.png)

#### 2.5.2 Пытаемся получить доступ к несуществующей странице first_page и получаем сообщение об ошибке

![picture](pic/pic2.png)


### 2.5.3 Переходим в elk и проверяем наличие сообщения об ошибке:
